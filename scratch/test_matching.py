...
for m in pattern.finditer(js_content):
    ...
    py = float(m.group(2))
    px = float(m.group(3))
    # Convert Leaflet coordinates (px, py) to SVG coordinates
    svg_x = 46.8 + 1.333333 * px
    svg_y = 2448.9 - 1.333333 * py
    plots_data.append({'id': pid, 'x': svg_x, 'y': svg_y, 'orig_x': px, 'orig_y': py})
...
# Match using converted SVG coordinates
matches = {}
for p in plots_data:
    px = p['x']
    py = p['y']
...
avg_d = sum(distances) / len(plots_data)
print(f"Average match distance after coordinate conversion: {avg_d:.2f} pixels")
