...
transform_fn = lambda xl, yl: (width_svg - yl, height_svg - xl)
...
for p in plots_data:
    ...
    for poly in reconstructed_polys:
        ...
        if d_val < min_d:
            min_d = d_val
            best_poly = poly
    matches.append({'id': p['id'], 'dist': min_d, 'cx': best_poly['cx'], 'cy': best_poly['cy'], 'tx': tx, 'ty': ty})

# Sort by distance
matches.sort(key=lambda x: x['dist'])

print("Top 30 matches under Option D (sorted by distance):")
for m in matches[:30]:
    print(f"  Plot {m['id']}: dist = {m['dist']:.2f} px, Target = ({m['tx']:.1f}, {m['ty']:.1f}), Centroid = ({m['cx']:.1f}, {m['cy']:.1f})")

print("\nWorst 30 matches under Option D:")
for m in matches[-30:]:
    print(f"  Plot {m['id']}: dist = {m['dist']:.2f} px, Target = ({m['tx']:.1f}, {m['ty']:.1f}), Centroid = ({m['cx']:.1f}, {m['cy']:.1f})")
