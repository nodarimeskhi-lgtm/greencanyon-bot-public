...
def parse_path_d(d_string):
    ...
    while i < len(tokens):
        ...
        try:
            ...
            raise ValueError(f"Error at token index {i} under command '{cmd_char}': {ex}")
...
c = Counter(lens)
print("\nDistribution of path lengths (number of vertices):")
for length, count in sorted(c.items(), key=lambda x: x[0]):
    if count > 5:
        print(f"  {length} vertices: {count} paths")
